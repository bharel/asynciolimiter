...
class FastFunc:
    def __init__(self, limiter: _CommonLimiterMixin) -> None:
        ...
        assert limiter
        self.queue: asyncio.Queue[Awaitable[Any]] = asyncio.Queue()
...
async def perform_test(to_be_called: FastFunc, num_calls: int, expected_duration: float) -> None:
    ...
    results = await asyncio.gather(
            asyncio.create_task(consume_updates(rate_limiter=to_be_called.limiter, q=to_be_called.queue)),
...
def test_LeakyBucketLimiterWorking():
    ...
    async def my_test() -> None:
        ...
        await perform_test(to_be_called, num_calls=num_calls, expected_duration=1.0)
    asyncio.run(my_test())
